...
class EKF(object):
    ...
    def transition_update(self, u, dt):
        ...
        self.x = g
        Pprev = self.P
        self.P = (np.dot(Gx, np.dot(Pprev, Gx.T)) + 
            dt * np.dot(Gu, np.dot(self.Q, Gu.T)))
        ##############
    ...
    def measurement_update(self, rawZ, rawR):
        ...
        self.x = self.x + np.dot(K, z).reshape(-1)
...
class Localization_EKF(EKF):
    ...
    def transition_model(self, u, dt):
        ...
        #### DONE ####
        if np.abs(om) > 1e-6:
          g =  np.array([x + v / om * (np.sin(th + dt * om) - sin(th)),
                         y + v / om * (-np.cos(th + dt * om) + cos(th)),
                         th + dt * om])
          Gx = np.array([[1, 0, v / om * (np.cos(th + dt * om) - np.cos(th))],
                         [0, 1, v / om * (np.sin(th + dt * om) - np.sin(th))],
                         [0, 0, 1]])
          Gu1 = [1.0 / om * (np.sin(th + dt * om) - np.sin(th)), 
            (dt*np.cos(th+dt*om)*v)/om-((np.sin(th+dt*om)-np.sin(th))*v)/om**2]
          Gu2 = [1.0 / om * (np.cos(th) - np.cos(th + dt * om)),
            (dt*np.sin(th+dt*om)*v)/om-((np.cos(th)-np.cos(th+dt*om))*v)/om**2]
          Gu3 = [0, dt]
          Gu = np.array([Gu1, Gu2, Gu3])
        else:
          g = np.array([x + dt * v * np.cos(th),
                        y + dt * v * np.sin(th),
                        th + dt * om])
          Gx = np.array([[1, 0, -dt * v * np.sin(th)],
                         [0, 1, dt * v * np.cos(th)],
                         [0, 0, 1]])
          Gu = np.array([[dt * np.cos(th), 0],
                         [dt * np.sin(th), 0],
                         [0, dt]])
        ##############
    ...
    def map_line_to_predicted_measurement(self, m):
        ...
        (x, y, th) = self.x
        dxc = xc * np.cos(th) - yc * np.sin(th)
        dyc = xc * np.sin(th) + yc * np.cos(th)

        h = np.zeros(2)
        h[0] = alpha - th - thc
        h[1] = r - ((x + dxc) * np.cos(alpha) + (y + dyc) * np.sin(alpha))
        ...
        Hx[1, 0] = -cos(alpha)
        Hx[1, 1] = -sin(alpha)
        Hx[1, 2] = ((-np.sin(alpha) * (np.cos(th) * xc - np.sin(th) * yc)) -
            np.cos(alpha) * ((-np.cos(th) * yc) - np.sin(th) * xc))
        # compute h, Hx
        ##############
    ...
    #  v_list - list of at most I innovation vectors (predicted map measurement
    #  - scanner measurement)
    #  R_list - list of len(v_list) covariance matrices of the innovation
    #  vectors (from scanner uncertainty)
    ...
    def associate_measurements(self, rawZ, rawR):
        ...
        R_list = []
        H_list = []
        map_n = self.map_lines.shape[1]
        (h_all, Hx_all) = zip(*[self.map_line_to_predicted_measurement(
          self.map_lines[:, i]) for i in range(map_n)])
        S_all = [np.dot(Hx_all[i], np.dot(self.P, Hx_all[i].T)) for i in
            range(map_n)]
        for i in range(rawZ.shape[1]):
          z = rawZ[:, i]
          R = rawR[i]
          v_all = [z - h_all[i] for i in range(map_n)]
          d = [np.dot(v_all[i], np.dot(np.linalg.inv(S_all[i] + R), v_all[i]))
              for i in range(map_n)]
          idx = np.argmin(d)
          d_min = d[idx]

          if d_min < self.g**2:
            v_list.append(v_all[idx])
            R_list.append(R)
            H_list.append(Hx_all[idx])
        ##############
    ...
    def measurement_model(self, rawZ, rawR):
        ...
        #### DONE ####
        # compute z, R, H
        z = np.vstack([v.reshape((-1, 1)) for v in v_list])
        R = sp.linalg.block_diag(*R_list)
        H = np.vstack(H_list)
        ##############
...
class SLAM_EKF(EKF):
    ...
    def transition_model(self, u, dt):
        ...
        if np.abs(om) > 1e-6:
          g_ =  np.array([x + v / om * (np.sin(th + dt * om) - sin(th)),
                         y + v / om * (-np.cos(th + dt * om) + cos(th)),
                         th + dt * om])
          Gx_ = np.array([[1, 0, v / om * (np.cos(th + dt * om) - np.cos(th))],
                          [0, 1, v / om * (np.sin(th + dt * om) - np.sin(th))],
                          [0, 0, 1]])
          Gu1_ = [1.0 / om * (np.sin(th + dt * om) - np.sin(th)), 
            (dt*np.cos(th+dt*om)*v)/om-((np.sin(th+dt*om)-np.sin(th))*v)/om**2]
          Gu2_ = [1.0 / om * (np.cos(th) - np.cos(th + dt * om)),
            (dt*np.sin(th+dt*om)*v)/om-((np.cos(th)-np.cos(th+dt*om))*v)/om**2]
          Gu3_ = [0, dt]
          Gu_ = np.array([Gu1_, Gu2_, Gu3_])
        else:
          g_ = np.array([x + dt * v * np.cos(th),
                         y + dt * v * np.sin(th),
                         th + dt * om])
          Gx_ = np.array([[1, 0, -dt * v * np.sin(th)],
                          [0, 1, dt * v * np.cos(th)],
                          [0, 0, 1]])
          Gu_ = np.array([[dt * np.cos(th), 0],
                          [dt * np.sin(th), 0],
                          [0, dt]])
        g[0:3] = g_
        Gx[0:3, 0:3] = Gx_
    ...
    # Localization_EKF.
    # In particular, essentially the only thing that needs to change is the
    # computation
    # of Hx in map_line_to_predicted_measurement and how that method is called
    # in
    # associate_measurements (i.e., instead of getting world-frame line
    # parameters from
    ...
    def measurement_model(self, rawZ, rawR):
        ...
        #### TODO ####
        # compute z, R, H (should be identical to
        # Localization_EKF.measurement_model above)
        z = np.vstack([v.reshape((-1, 1)) for v in v_list])
        R = sp.linalg.block_diag(*R_list)
        H = np.vstack(H_list)
        ##############
    ...
    #
    # Note that instead of the actual parameters m = (alpha, r) we pass in the
    # map line index j so that we know which components of the Jacobian to fill
    # in.
    def map_line_to_predicted_measurement(self, j):
        ...
        Hx_[1, 0] = -cos(alpha)
        Hx_[1, 1] = -sin(alpha)
        Hx_[1, 2] = ((-np.sin(alpha) * (np.cos(th) * xc - np.sin(th) * yc)) -
            np.cos(alpha) * ((-np.cos(th) * yc) - np.sin(th) * xc))
        ...
        if j > 1:
            Hx[0, 3+2*j] = 1.0
            Hx[1, 3+2*j] = (np.sin(alpha) * (x + dxc) - np.cos(alpha) * (y +
              dyc))
            Hx[0, 3+2*j+1] = 0.0
            Hx[1, 3+2*j+1] = 1.0
        ##############
    ...
    def associate_measurements(self, rawZ, rawR):
        ...
        R_list = []
        H_list = []
        map_n = (self.x.size - 3) // 2
        (h_all, Hx_all) = zip(*[self.map_line_to_predicted_measurement(i) 
          for i in range(map_n)])
        S_all = [np.dot(Hx_all[i], np.dot(self.P, Hx_all[i].T)) for i in
            range(map_n)]
        for i in range(rawZ.shape[1]):
