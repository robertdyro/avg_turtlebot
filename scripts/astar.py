...
class AStar(object):
    ...
    def __init__(self, statespace_lo, statespace_hi, x_init, x_goal, occupancy, resolution=1):
        ...
        self.g_score[self.x_init] = 0
        self.f_score[self.x_init] = self.distance(self.x_init,self.x_goal)
    ...
    def get_neighbors(self, x):
        ...
        for delta_x in deltas:
            for delta_y in deltas:
                test_neighbor = self.snap_to_grid((x[0] + delta_x * self.resolution, x[1] + delta_y * self.resolution)) #
                #import pdb; pdb.set_trace()
                if self.is_free(test_neighbor) == True and test_neighbor != x:
                    neighbors.append(test_neighbor)
        return neighbors
    ...
    def solve(self):
        while len(self.open_set)>0:
            ...
            for neighbor in self.get_neighbors(x_cur):
                if neighbor in self.closed_set:
                    continue
                #import pdb; pdb.set_trace()
                tentative_g_score = self.g_score[x_cur] + self.distance(x_cur, neighbor)
